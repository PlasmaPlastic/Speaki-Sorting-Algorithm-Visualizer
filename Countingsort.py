...
pygame.display.set_caption("Counting Sort - dot.mp3 + spiki.mp3")
...
def counting_sort(arr):
    ...
    for i in range(len(count)):
        while count[i] > 0:
            for j in range(k, n):
                comps += 1
                yield k, j, comps, swaps, "compare"
                if arr[j]["idx"] == i:
                    if j != k:
                        arr[k], arr[j] = arr[j], arr[k]
                        swaps += 1
                        play_dot_sound(arr[k]["idx"])
                        yield k, j, comps, swaps, "swap"
                    k += 1
                    break
            count[i] -= 1
ALGO_FUNC = counting_sort
...
while running:
    ...
    info = f"Counting Sort  Comparisons: {comparisons} | Swaps: {swaps} | Time: {elapsed:.2f}s  [R] reset"
