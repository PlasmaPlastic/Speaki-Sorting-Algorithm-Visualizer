...
pygame.display.set_caption("Heap Sort - dot.mp3 + spiki.mp3")
...
def heap_sort(arr):
    ...
    def heapify(n, i):
        ...
        if l < n:
            comps[0] += 1
            yield l, largest, comps[0], swaps[0], "compare"
            if arr[l]["idx"] > arr[largest]["idx"]:
                largest = l
        if r < n:
            comps[0] += 1
            yield r, largest, comps[0], swaps[0], "compare"
            if arr[r]["idx"] > arr[largest]["idx"]:
                largest = r
        if largest != i:
            arr[i], arr[largest] = arr[largest], arr[i]
            swaps[0] += 1
            play_dot_sound(arr[i]["idx"])
...
while running:
    ...
    info = f"Heap Sort  Comparisons: {comparisons} | Swaps: {swaps} | Time: {elapsed:.2f}s  [R] reset"
