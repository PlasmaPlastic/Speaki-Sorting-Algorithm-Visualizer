...
pygame.display.set_caption("Radix Sort - dot.mp3 + spiki.mp3")
...
def radix_sort(arr):
    comps = 0
    swaps = 0
    max_idx = max(x["idx"] for x in arr)
    exp = 1
    n = len(arr)
    while max_idx // exp > 0:
        output = [None] * n
        count = [0] * 10
        for i in range(n):
            digit = (arr[i]["idx"] // exp) % 10
            count[digit] += 1
            comps += 1
            yield i, digit, comps, swaps, "compare"
        for i in range(1, 10):
            count[i] += count[i - 1]
        for i in range(n - 1, -1, -1):
            ...
            yield i, count[digit], comps, swaps, "swap"
        for i in range(n):
            arr[i] = output[i]
        exp *= 10
ALGO_FUNC = radix_sort
...
while running:
    ...
    info = f"Radix Sort  Comparisons: {comparisons} | Swaps: {swaps} | Time: {elapsed:.2f}s  [R] reset"
