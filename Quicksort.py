...
pygame.display.set_caption("Quick Sort - dot.mp3 + spiki.mp3")
...
def quick_sort(arr):
    ...
    def _qs(low, high):
        ...
        for j in range(low, high):
            comps[0] += 1
            yield j, high, comps[0], swaps[0], "compare"
            if arr[j]["idx"] < pivot:
                arr[i], arr[j] = arr[j], arr[i]
                swaps[0] += 1
                play_dot_sound(arr[i]["idx"])
                yield i, j, comps[0], swaps[0], "swap"
                i += 1
        arr[i], arr[high] = arr[high], arr[i]
        swaps[0] += 1
        play_dot_sound(arr[i]["idx"])
        yield i, high, comps[0], swaps[0], "swap"
        yield from _qs(low, i - 1)
        yield from _qs(i + 1, high)
    yield from _qs(0, len(arr) - 1)
ALGO_FUNC = quick_sort
...
while running:
    ...
    info = f"Quick Sort  Comparisons: {comparisons} | Swaps: {swaps} | Time: {elapsed:.2f}s  [R] reset"
