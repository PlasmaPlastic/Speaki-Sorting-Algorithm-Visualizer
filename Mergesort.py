...
pygame.display.set_caption("Merge Sort - dot.mp3 + spiki.mp3")
...
def merge_sort(arr):
    ...
    def _merge_sort(l, r):
        ...
        while i < len(left) and j < len(right):
            comps[0] += 1
            yield l + i, m + j, comps[0], swaps[0], "compare"
            ...
            k += 1
            yield k - 1, k, comps[0], swaps[0], "swap"
        while i < len(left):
            arr[k] = left[i]
            i += 1
            k += 1
...
while running:
    ...
    info = f"Merge Sort  Comparisons: {comparisons} | Swaps: {swaps} | Time: {elapsed:.2f}s  [R] reset"
